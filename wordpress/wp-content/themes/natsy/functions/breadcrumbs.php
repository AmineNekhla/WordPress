...
function nova_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'natsy' ),
			'archive'   => esc_html__( 'Archives', 'natsy' ),
			'blog'      => esc_html__( 'Blog', 'natsy' ),
			'search'    => esc_html__( 'Search results for', 'natsy' ),
			'not_found' => esc_html__( 'Not Found', 'natsy' ),
			'author'    => esc_html__( 'Author Archives:', 'natsy' ),
			'day'       => esc_html__( 'Daily Archives:', 'natsy' ),
			'month'     => esc_html__( 'Monthly Archives:', 'natsy' ),
			'year'      => esc_html__( 'Yearly Archives:', 'natsy' ),
		),
	) );
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'natsy' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'natsy' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'natsy' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	else {
		$items[] = sprintf(
