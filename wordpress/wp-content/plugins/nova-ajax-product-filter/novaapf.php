<?php
/**
 * Plugin Name: Nova Ajax Product Filters
...
if (!class_exists('NOVAAPF')) {
	class NOVAAPF
	{
		...
		{
			$this->define('NOVAAPF_LOCALE', $this->plugin_slug);
			$this->define('NOVAAPF_PATH', $this->pluginPath());
			$this->define('NOVAAPF_ASSETS_PATH', $this->assetsPath());
			$this->define('NOVAAPF_CACHE_TIME', 60*60*12);
		}
		...
		{
			$settings = get_option('novaapf_settings');

			wp_register_style('novaapf-style', NOVAAPF_ASSETS_PATH . 'css/novaapf-styles.css');

			wp_register_script('novaapf-script', NOVAAPF_ASSETS_PATH . 'js/scripts.js', array('jquery'), '20120206', true);
			wp_localize_script('novaapf-script', 'novaapf_price_filter_params', array(
				'currency_symbol' => get_woocommerce_currency_symbol(),
				'currency_pos'    => get_option('woocommerce_currency_pos')
			));

			if ($settings) {
				wp_localize_script('novaapf-script', 'novaapf_params', $settings);
			}

			wp_register_style('novaapf-nouislider-style', NOVAAPF_ASSETS_PATH . 'css/nouislider.min.css');
			wp_register_script('novaapf-nouislider-script', NOVAAPF_ASSETS_PATH . 'js/nouislider.min.js', array ('jquery'), '1.0', true);
			wp_register_script('novaapf-price-filter-script', NOVAAPF_ASSETS_PATH . 'js/price-filter.js', array ('jquery'), '1.0', true);

			wp_register_script('novaapf-select2', NOVAAPF_ASSETS_PATH . 'js/select2.min.js', array ('jquery'), '1.0', true);
		}
		...
		{
			add_options_page(__('Nova Ajax Product Filter', 'novaapf'), __('Nova Ajax Product Filter', 'novaapf'), 'manage_options', 'novaapf-settings', array($this, 'settingsPage'));
		}
		...
		{
			register_setting('novaapf_settings', 'novaapf_settings', array($this, 'validateSettings'));
		}
		...
		{
			if (!get_option('novaapf_settings')) {
				// check if filter is applied
				$settings = apply_filters('novaapf_settings', $this->defaultSettings());
				update_option('novaapf_settings', $settings);
			}
		}
		...
		{
			if (has_filter('novaapf_settings')) {
				$input = apply_filters('novaapf_settings', $input);
			}

			if (isset($input['clear_transients']) && $input['clear_transients'] == '1' ) {
				// clear transient
				novaapf_clear_transients();
			}
		...
		public function getChosenFilters()
		{
			...
			if (isset($_GET['orderby'])) {
				$orderby = (!empty($_GET['orderby'])) ? sanitize_sql_orderby($_GET['orderby']) : '';
			...
			foreach ($query as $key => $value) {
				...
				if (preg_match('/^attr/', $key) && !empty($query[$key])) {
					...
					foreach ($terms as $term_id) {
						$ancestors = novaapf_get_term_ancestors($term_id, $taxonomy);
						$term_data = novaapf_get_term_data($term_id, $taxonomy);
		...
		public function setFilter($q)
		{
			...
			if ($wp_query->is_sale_page) {
				if ( sizeof($post__in) == 0 ) {
					$post__in = array_merge($post__in, wc_get_product_ids_on_sale() );
				} else {
		...
		public function unfilteredProductIds()
		{
 			if (!is_tax(get_object_taxonomies('product'))) {
 				...
 				$transient_name = 'novaapf_unfiltered_product_ids';

 				if (false === ($unfiltered_product_ids = get_transient($transient_name))) {
 					$unfiltered_product_ids = get_posts($args);
 					set_transient($transient_name, $unfiltered_product_ids, novaapf_transient_lifespan());
 				}
		...
		public function unfilteredProductsPriceRange()
		{
			...
			$transient_name = 'novaapf_unfiltered_product_price_range';
			...
			if (false === ($unfiltered_products_price_range = get_transient($transient_name))) {
				...
				set_transient($transient_name, $unfiltered_products_price_range, novaapf_transient_lifespan());
			}
		...
		{
			echo '<div class="novaapf-before-products">';
		}
		...
		{
			load_plugin_textdomain('novaapf', FALSE, basename(dirname(__FILE__)) . '/languages/');
		}
		...
		public function needWoocommerce()
		{
			...
			echo '<p>' . __('Nova Ajax Product Filter needs WooCommerce plugin to work.', 'novaapf') . '</p>';
			echo '</div>';
		}
		...
		{
			echo '<div class="error">';
			echo '<p>' . __('To use Nova Ajax Product Filter update your WooCommerce plugin.', 'novaapf') . '</p>';
			echo '</div>';
		}
		...
		{
			$links[] = '<a href="' . admin_url('options-general.php?page=novaapf-settings') . '">' . __('Settings', 'novaapf') . '</a>';
			return $links;
		}
	}
}
 ...
 */
$GLOBALS['novaapf'] = NOVAAPF::instance();
