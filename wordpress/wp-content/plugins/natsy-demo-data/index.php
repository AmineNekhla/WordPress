<?php
/*
Plugin Name:    Natsy Package Demo Data
...
if(!function_exists('nova_import_get_post_id_by_slug')){
    function nova_import_get_post_id_by_slug( $slug, $post_type ){
        global $wpdb;
...
class Natsy_Data_Demo_Plugin_Class{
    ...
    public static $theme_name = 'natsy';

    public static $demo_site = 'https://natsy.novaworks.net/';
    ...
    public static function isLocal(){
        ...
        if ( strpos($hostname, '.novaworks.net') !== false || strpos($hostname, 'localhost') !== false ) {
            $is_local = true;
        }
        return $is_local;
    }
...
add_action('plugins_loaded', function(){
    ...
    if(strtolower($theme->get_template()) != 'natsy'){

        add_action( 'admin_notices', function(){
            printf( __( '%1$s"Natsy Package Demo Data" requires %3$s"natsy"%4$s theme to be installed and activated. Please active %3$s"natsy"%4$s to continue.%2$s', 'novaworks-demodata' ), '<div class="error"><p>', '</p></div>' ,'<strong>', '</strong>' );
        });
    ...
    Natsy_Data_Demo_Plugin_Class::get_instance();
}, 999);
